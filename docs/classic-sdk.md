...
ここで目指すのは、対立する仮説を混ぜて無難なグレーへすることではありません。明確な仮説と反証を別々の石として置き、局所的な明確さと盤面全体の多元性を両立させます。この原理を **No Gray Stones** と呼びます。pull、clone、forkされた環境も独立した生育環境となり、同じ祖先から複数のWorldlineを育てられます。概念、世代、分散育成、現行実装の境界は[`docs/worldlines.md`](worldlines.md)を参照してください。
